...
def test_fp32_block_gate_uses_piecewise_silu() -> None:
    block = EMambaBlock(d_model=2, expand=1, d_state=1)
    with torch.no_grad():
    ...
    expected = tokens + normalized * piecewise_silu(normalized * 0.5)
    torch.testing.assert_close(block(tokens), expected, rtol=0, atol=0)


def test_fp32_ssm_uses_piecewise_exp_for_abar() -> None:
    ssm = SelectiveSSM(d_inner=1, d_state=1, dt_rank=1)
    with torch.no_grad():
    ...
    decay = piecewise_exp(torch.tensor(-0.25))
